...
<head>
    ...
    <title>Operations Project Tracking - PFIMS</title>
    <link rel="stylesheet" href="{{ asset('css/Oprojects.css') }}">
        ...
        .status-chart-row { display:grid; grid-template-columns:80px minmax(0,1fr) 32px; gap:8px; align-items:center; font-size:.78rem; }
...
<body class="projects-page">
    ...
    <aside class="sidebar">
            ...
            <ul>
                ...
                <li class="active"><a href="{{ url('/oprojects') }}"><img src="{{ asset('images/projects.png') }}" alt="" class="nav-link-icon">PROJECTS</a></li>
    ...
    if (editStartDate) {
        editStartDate.addEventListener('change', function() {
            ...
            if (end && new Date(end) > new Date(this.value)) {
                ...
                showFieldError('editEstEndDate', 'editEstEndDateError', 'Estimated end date must be after the start date.');
            }
        });
    }
    ...
    if (editActualEndDate) {
        editActualEndDate.addEventListener('change', function() {
            ...
            if (this.value > todayStr) {
                showFieldError('editActualEndDate', 'editActualEndDateError', 'Actual end date cannot be in the future.');
            } else if (start && new Date(this.value) < new Date(start)) {
                showFieldError('editActualEndDate', 'editActualEndDateError', 'Actual end date cannot be before the start date.');
            } else {
                clearFieldError('editActualEndDate', 'editActualEndDateError');
            }
        });
    ...
    if (editCompletionPercentage) {
        ...
        function closeEditProjectModal() {
            ...
            if (completion !== '' && (parseFloat(completion) < 0 || parseFloat(completion) > 100)) {
                showFieldError('editCompletionPercentage', 'editCompletionPercentageError', 'Completion percentage must be between 0 and 100.');
                return;
            }
        ...
        document.getElementById('editProjectModal').addEventListener('click', function(e) {
            ...
            if (suppressErrorAutoClose) {
                suppressErrorAutoClose = false;
                return;
            }
