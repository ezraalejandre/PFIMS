...
<body class="reports-page" data-portal="{{ $portal }}" data-reports-api-base="{{ request()->getBaseUrl() }}/api/reports">
    ...
    <dialog id="historyDetailDialog" class="export-dialog history-detail-dialog" aria-labelledby="historyDetailTitle">
        <form method="dialog" class="history-detail-content">
            <div class="dialog-heading">
                ...
                <button type="button" class="icon-btn" id="closeHistoryDetail" aria-label="Close export details">×</button>
            </div>
            ...
            <div class="dialog-actions">
                <button type="button" class="btn btn-secondary" id="cancelHistoryDetail">Cancel</button>
            </div>
        </form>
    </dialog>
        ...
        (() => {
            ...
            async function loadDataset() {
                ...
                try {
                    ...
                    state.payload = await apiJson(`${reportsApiBase}/data/${state.dataset}?${queryString(query)}`);
            ...
            function renderChart() {
                ...
                state.chartInstance = new Chart(canvas, {
                    ...
                    options: {
                        ...
                        interaction: { mode: isPie ? 'nearest' : 'index', intersect: true },
                        plugins: { legend: { position: 'bottom' }, tooltip: { callbacks: {
                            label: context => `${context.label}: ${context.dataset.label}: ${new Intl.NumberFormat('en-PH', { maximumFractionDigits: 2 }).format(Number(context.raw || 0))}`
                        } } },
                        ...
                        ? reports.map(report => {
                            ...
                            return `
                                <tr>
                                    ...
                                    <td class="history-actions"><button type="button" class="table-link history-view-button" data-report-id="${escapeHtml(report.report_id)}">View details</button><a class="table-link" href="${reportsApiBase}/download/${encodeURIComponent(report.report_id)}">Download</a></td>
                                </tr>
                            `;
                        }).join('')
            ...
            document.getElementById('cancelHistoryDetail').addEventListener('click', closeHistoryDetails);
